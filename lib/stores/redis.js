...
exports.mixin = function(model) {
  ...
  model.prototype.linkCount = function(type, cb) {
    model.client.scard(this.key() +":"+type, function(err, data) {
      cb ? cb(data) : null
      // if(err) return cb(false)
  ...
  model.prototype.link = function(id, type, cb) {
    model.client.SADD(this.key() +":"+type, id, function(err, data) {
      cb ? cb(!err) : null
    })
  }

  model.prototype.unlink = function(id, type, cb) {
    model.client.SREM(this.key() +":"+type, id, function(err, data) {
      cb ? cb(!err) : null
    })
  }
  ...
  model.prototype.destroy = function(cb) {
    ...
    model.q.add(function() {
      model.client.del(self.key(), function(err, data) {
        model.client.srem(klass +"s", self.id, function(err, data) {
          done()
        })
      })
    })
  ...
  model.prototype._save = function() {
    ...
    function getID(next) {
      ...
      if(self.id == null && model.properties.id.auto_inc) {
        client.INCR(klass + "s:id", function(err, data) {
          self.id = data
          next()
        })
      }
    ...
    function updateKeys(next) {
      client.sadd(klass + "s", self.id, next)
    }
  ...
  model.count = function(cb) {
    model.client.scard(model.klass+"s", function(err, data) {
      cb(data)
    })
  }
